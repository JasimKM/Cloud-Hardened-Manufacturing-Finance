...
ALLOWED_EXTENSIONS = set(os.getenv("ALLOWED_EXTENSIONS", ".nc,.gcode,.l5x,.stl,.dwg,.pdf,.log,.csv,.bin").split(','))
...
def encrypt():
    ...
    if not allowed_file(file.filename):
        flash(f'File type not allowed. Supported types: {", ".join(ALLOWED_EXTENSIONS)}', 'danger')
        return redirect(url_for('index'))
    ...
    try:
        ...
        # FIX: RFC 8187 compliant Content-Disposition
        rfc8187_filename = quote(enc_filename)
        return Response(
            encrypted_data,
            mimetype='application/octet-stream',
            headers={
                "Content-Disposition": f"attachment; filename=\"{enc_filename}\"; filename*=UTF-8''{rfc8187_filename}",
...
def decrypt():
    if request.method == 'POST':
        ...
        try:
            ...
            # FIX: RFC 8187 compliant Content-Disposition
            rfc8187_filename = quote(original_name)
            return Response(
                decrypted_data,
                mimetype='application/octet-stream',
                headers={
                    "Content-Disposition": f"attachment; filename=\"{original_name}\"; filename*=UTF-8''{rfc8187_filename}",
                    "Cache-Control": "no-cache, no-store, must-revalidate",
                    "Pragma": "no-cache",
                    "Expires": "0",
                    "Access-Control-Expose-Headers": "Content-Disposition",
